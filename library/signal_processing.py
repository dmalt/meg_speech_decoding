...
def remove_eyes_artifacts(signal: np.ndarray, sr: float) -> np.ndarray:
    return Filter(signal, sr).highpass(lowcut=10, order=5).signal


def remove_target_leakage(signal: np.ndarray, sr: float) -> np.ndarray:
    return Filter(signal, sr).lowpass(highcut=200, order=5).signal


def remove_silent_noise(sound: np.ndarray, sr: float) -> np.ndarray:
...
class Filter:
    ...
    def moving_avarage(self, window: int) -> "Filter":
        w2 = int(window // 2)
        pad = np.pad(self.signal, (w2, w2), "constant", constant_values=0)
        pad_cumsum = np.cumsum(pad)
        self.signal = (pad_cumsum[w2 * 2 :] - pad_cumsum[: -w2 * 2]) / window
        return self

    def log_envelope(self, highcut=200, lowcut=200) -> "Filter":
    ...
    def notch_narrow(self, freq) -> "Filter":
        return self.notch(freq, freq)

    def notch_wide(self, freq) -> "Filter":
...
class SpectralFeatures:
    ...
    def asd(self, n: Optional[int] = None) -> tuple[np.ndarray, np.ndarray]:
        """
    ...
    def mfccs(self, d: int, out_nsamp: int, n_mfcc: int) -> np.ndarray:
        m = lbf.mfcc(y=self.signal, sr=self.sr, hop_length=d, n_mfcc=n_mfcc)
        mfccs_resampled = scs.resample(x=m.T, num=out_nsamp)
        return mfccs_resampled

    def logmelspec(self, n: int, f_max: float, d: int) -> tuple[np.ndarray, float]:
    ...
    def lpcs(self, order, window, d: int, out_nsamp: int) -> np.ndarray:
        w2 = window // 2
        pad = np.pad(self.signal, (w2, w2))
        res = [lb.lpc(pad[i - w2 : i + w2 + 1], order) for i in range(w2, len(pad) - w2 + 1, d)]
        res = np.array(res)[:, 1:]
