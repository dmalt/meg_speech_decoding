...
from .runner_common import (CLASSIFICATION_MODE, MODEL_DUMPS_DIR,
                            REGRESSION_MODE, RESULTS_DIR, WORDS_REMAP,
                            data_generator)
...
def process_batch(
    bench_model, generator, is_train, iteration, max_words_length
):
    ...
    if len(non_silent_indexes) > 0:
        metrics["accuracy (without silent class)"] = float(
            np.mean(
                y_predicted_numpy[non_silent_indexes]
                == y_batch_numpy[non_silent_indexes]
            )
        )
...
def load_words_info(filepath):
    ...
    with open(filepath) as phrases_file:
        for row in phrases_file:
            ...
            assert (
                splitted_row[2] in WORDS_REMAP
            ), f"phrase {splitted_row[2]} not allowed"
            phrases_info.append(splitted_row)
    return phrases_info


def get_random_predictions(
    bench_model, generator, iterations, max_words_length
):
    Y_batch = []
    Y_predicted = []
    for index, (x_batch, y_batch) in enumerate(generator):
        ...
        if torch.cuda.is_available():
            x_batch = x_batch.cuda()
        y_predicted = (
            bench_model.model(x_batch).cpu().detach().numpy().argmax(axis=1)
        )
...
def predict_regression(bench_model_regression, ecog):
    ...
    for ecog_batch in all_data_generator:
        ...
        x_predicted = (
            bench_model_regression.model(ecog_batch).cpu().data.numpy()
        )
...
def prepare_frames(x, words_info, downsampling_coef):
    ...
    for phrase_start, phrase_end, phrase in words_info:
        if last_phrase_end != 0 and last_phrase_end != phrase_start:
            ...
            x_frames.append(
                x[
                    int(last_phrase_end / downsampling_coef) : int(
                        phrase_start / downsampling_coef
                    )
                ]
            )
        ...
        x_frames.append(
            x[
                int(phrase_start / downsampling_coef) : int(
                    phrase_end / downsampling_coef
                )
            ]
        )
...
def fix_class_imbalance(X, Y):
    ...
    selected_indexes = np.sort(
        np.concatenate([selected_silent_indexes, non_silent_indexes])
    )
...
def random_iterator(X, Y, batch_size=1):
    ...
    while True:
        ...
        yield copy.deepcopy(X[current_indexes]), copy.deepcopy(
            Y[current_indexes]
        )
...
def run_classification(regression_bench_model_name, patient, is_debug=False):
    ...
    bench_regression_model = getattr(
        bench_models_regression, regression_bench_model_name
    )(patient)
    ...
    for filepath in patient["files_list"]:
        ...
        x = bench_regression_model.preprocess_ecog(
    ...
    for regression_model_filename in all_models_files:
        ...
        regression_model_file_path = (
            f"{MODEL_DUMPS_DIR}/{regression_model_filename}"
        )
        ...
        bench_regression_model.model.load_state_dict(
            torch.load(regression_model_file_path)
        )
        ...
        for index, filepath in enumerate(patient["files_list"]):
            ...
            x = predict_regression(bench_regression_model, ecog).astype(
                "float32"
            )
        ...
        for iteration in range(max_iterations_count):
            process_batch(
                bench_model, train_generator, True, iteration, max_words_length
            )
            with torch.no_grad():
